...
def get_score(review, posterior_prob_dict, class_prior_prob):
    # prior probability
    score_class = class_prior_prob
    for word in review:
        score_class += posterior_prob_dict.get(word) if word in posterior_prob_dict.keys() else posterior_prob_dict.get('$UNKNOWN_WORD_DEFAULT')
    return score_class


def classify(positive_truthful_score, positive_deceptive_score, negative_truthful_score, negative_deceptive_score):
...
for file in testing_files:
    ...
    words = line.split()
    review = remove_stop_words(words)

    positive_truthful_score = get_score(review, positive_truthful_conditional_prob_dict, positive_truthful_prior_prob)
    positive_deceptive_score = get_score(review, positive_deceptive_conditional_prob_dict, positive_deceptive_prior_prob)
    negative_truthful_score = get_score(review, negative_truthful_conditional_prob_dict, negative_truthful_prior_prob)
    negative_deceptive_score = get_score(review, negative_deceptive_conditional_prob_dict, negative_deceptive_prior_prob)

    prediction = classify(positive_truthful_score, positive_deceptive_score, negative_truthful_score, negative_deceptive_score)
