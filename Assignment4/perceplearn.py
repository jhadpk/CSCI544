...
def initialize():
    for word in features:
        weights_polarity[word] = 0
        weights_origin[word] = 0
...
# for feature classification - negative or positive
weights_polarity = {}
bias_polarity = 0

# for feature classification - truthful or deceptive
weights_origin = {}
bias_origin = 0

# used for average perceptron  http://www.ciml.info/dl/v0_99/ciml-v0_99-ch04.pdf Pg 13
weights_cached_polarity = {}
...
for k in range(0, 10):
    ...
    for file in all_files:
        activation_polarity = 0
        activation_origin = 0
        y_polarity = 1 if "positive_polarity" in file else -1
        y_origin = 1 if "truthful_from" in file else -1
        word_count_dict = get_word_count(file)
        for word in word_count_dict:
            if weights_polarity[word] != 0:
                activation_polarity += weights_polarity[word] * word_count_dict[word]
                activation_origin += weights_origin[word] * word_count_dict[word]

        activation_polarity = update_activation(activation_polarity, bias_polarity)
        if y_polarity * activation_polarity <= 0:
            for word in word_count_dict:
                weights_polarity[word] = update_weight(weights_polarity[word], y_polarity, word_count_dict[word])
                weights_cached_polarity[word] = update_cache_weight(weights_cached_polarity[word], y_polarity, c, word_count_dict[word])
            bias_polarity = update_bias(bias_polarity, y_polarity)
            bias_cached_polarity = update_cache_bias(bias_cached_polarity, y_polarity, c)

        activation_origin = update_activation(activation_origin, bias_origin)
        if y_origin * activation_origin <= 0:
            for word in word_count_dict:
                weights_origin[word] = update_weight(weights_origin[word], y_origin, word_count_dict[word])
                weights_cached_origin[word] = update_cache_weight(weights_cached_origin[word], y_origin, c, word_count_dict[word])
            bias_origin = update_bias(bias_origin, y_origin)
...
for i in weights_polarity:
    ...
    weights_origin[i] = get_averaged_value(weights_origin[i], weights_cached_origin[i], c)
